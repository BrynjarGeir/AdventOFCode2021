...
def checkProjection(scanner0, scanner1):
    try :
        ...
        x0 = prev[0][2][0]; x1 = prev[1][0][0]; y0 = prev[0][2][1]; y1 = prev[1][0][1]; z0 = prev[0][2][2]; z1 = prev[1][0][2]
        ...
        if abs(x0Diff) == abs(x1Diff):
            if signs[0]: projections.append(('x',1,x1-x0))
            else: projections.append(('x', -1, x1-x0))
        elif abs(x0Diff) == abs(y1Diff):
            if signs[0]: projections.append(('y',1,y1-x0))
            else: projections.append(('y', -1,y1-x0))
        elif abs(x0Diff) == abs(z1Diff):
            if signs[0]: projections.append(('z',1,z1-x0))
            else: projections.append(('z', -1,z1-x0))
        else: print('Points Not Matching!')
        if abs(y0Diff) == abs(x1Diff):
            if signs[1]: projections.append(('x',1,x1-y0))
            else: projections.append(('x', -1,x1-y0))
        elif abs(y0Diff) == abs(y1Diff):
            if signs[1]: projections.append(('y',1,y1-y0))
            else: projections.append(('y', -1,y1-y0))
        elif abs(y0Diff) == abs(z1Diff):
            if signs[1]: projections.append(('z',1,z1-z0))
            else: projections.append(('z', -1,z1-z0))
        else: print('Points Not Matching!')
        if abs(z0Diff) == abs(x1Diff):
            if signs[2]: projections.append(('x',1,x1-z0))
            else: projections.append(('x', -1,x1-z0))
        elif abs(z0Diff) == abs(y1Diff):
            if signs[2]: projections.append(('y',1,y1-z0))
            else: projections.append(('y', -1,z0-y1))
        elif abs(z0Diff) == abs(z1Diff):
            if signs[2]: projections.append(('z',1,z1-z0))
            else: projections.append(('z', -1,z1-z0))
        else: print('Points Not Matching!')
...
def removeUnProjections(allProjections):
    ...
    for item in allProjections:
        if item != None and set((item[1][0], item[1][1])) not in completedProjections:
...
# I also have to keep track of which is x,y,z
def totalProjection(allProjections, projectionPaths):
    ...
    for scanner in projectionPaths:
        if len(projectionPaths[scanner]) == 2:
            x = [first for first in allProjections[(0,scanner)] if first[0]=='x'][0]
            y = [second for second in allProjections[(0,scanner)] if second[0] == 'y'][0]
            z = [third for third in allProjections[(0,scanner)] if third[0] == 'z'][0]
            scannerProjection = {'x':x[2], 'y':y[2], 'z':z[2]}
            actualScannerProjection[scanner] = scannerProjection
        else:
            path = projectionPaths[scanner].copy()
            scannerProjection = {'x':0, 'y':0, 'z':0}
            while len(path)>1:
                currProjection = allProjections[(path[0],path[1])]
                x = [first for first in currProjection if first[0]=='x'][0]
                y = [second for second in currProjection if second[0] == 'y'][0]
                z = [third for third in currProjection if third[0] == 'z'][0]
                scannerProjection['x'] += x[2]; scannerProjection['y'] += y[2]; scannerProjection['z'] += z[2]; 
                path.pop(0)
            actualScannerProjection[scanner] = scannerProjection
...
# Converting every datapoint to zero coordinates and add to single list rather than dict of scanners
def convertToZero(totalProjections, data):
    beacons = []
    for scanner in data:
            for beacon in data[scanner]:
                if scanner != 'scanner 0':
                    i = int(''.join(filter(str.isdigit, scanner)))
                    curr = totalProjections[i]
                    x = curr['x']; y = curr['y']; z = curr['z']
                    beacons.append((beacon[0] - x, beacon[1] - y, beacon[2] - z))
                else:
                    beacons.append((beacon[0], beacon[1], beacon[2]))
    return beacons
...
totalProjections = totalProjection(allProjections, projectionPaths)
print('Finished the total projections')

# I mean there are definitely errors here but the next step if this is all correct is to convert all points to 0 coordinates
zeroPoints = set(convertToZero(totalProjections, data))
print('Finished Converting to Zero Points')

exampleResults = open('../data/exampleResults').readlines()
exampleResults = [point.strip().split(',') for point in exampleResults]
exampleResults = [(int(point[0]), int(point[1]), int(point[2])) for point in exampleResults]
exampleResults = set(exampleResults)


print((1889, -1729, 1762) in zeroPoints)
#pp.pprint(allProjections)
#pp.pprint(projectionPaths)
#pp.pprint(totalProjections)
#pp.pprint(len(zeroPoints))