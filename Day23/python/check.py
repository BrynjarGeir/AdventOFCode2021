...
class Room:
    ...
    def is_complete(self):
        ...
        if self.type == E: return True
        for crab in self.pos:
            if crab != self.type: return False
        return True
    ...
    def is_empty(self):
        ...
        for i in range(self.s):
            ...
            # we ignore them
            if self.pos[i] == self.type: continue
            if self.pos[i]: return False
        return True
    ...
    def get_next(self):
        ...
        for i in range(self.s):
            if self.pos[i]: return self.pos[i], i
    ...
    def get_position(self):
        ...
        for i in range(self.s - 1, -1, -1):
            if not self.pos[i]: return i + 1

    def __eq__(self, other: 'Room'):
        for j in range(self.s):
            if self.pos[j] != other.pos[j]: return False
        return True

class State:
    def __init__(self, part_one=True):
        self.rooms = []
        if part_one: self.room_depths = 2
        else: self.room_depths = 4
    ...
    def count_completed_rooms(self):
        ...
        self.rooms_complete = 0
        for i in [1, 3, 5, 7]:
            if self.rooms[i].is_complete(): self.rooms_complete += self.rooms[i].type

    def __lt__(self, other: 'State'):
        # For heapq
        # See who has more rooms complete
        # Favor rooms that have more expensive crabs already tucked.
        return self.rooms_complete < other.rooms_complete
    ...
    def create_rooms(self):
        ...
        for i in range(9):
            ...
            if i % 2 == 0:
                ...
                room.pos = [identifier[i] for _ in range(self.room_depths)]
    ...
    def __eq__(self, other: 'State'):
        ...
        """
        eq = True
        for i in [1, 3, 5, 7]: eq &= self.rooms[i] == other.rooms[i]
        return eq
    ...
    def __hash__(self):
        ...
        for room in self.rooms:
            for pos in room.pos: string += enum2char[pos]
        return hash(string)
...
# Part 1
if part_one:
    current_state.rooms[1].pos = [D, B]
    current_state.rooms[3].pos = [A, C]
    current_state.rooms[5].pos = [C, B]
...
def dict_def_value(): return 100_000_000
